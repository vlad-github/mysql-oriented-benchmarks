...
for m in seqrd rndrd seqwr ; do for t in 1 2 4 6 8 12 16 24 32 48 64 98 128 190 256 384 512; do echo "Test: $m threads: $t" ; 
...
for m in rndwr ; do for t in 1 2 4 6 8 12 16 24 32 48 64 98 128 190 256 384 512; do echo "Test: $m threads: $t" ; 
    sysbench --test=fileio --file-io-mode=sync --file-total-size=20G --file-extra-flags=direct --file-test-mode=$m --time=30 --threads=$t run > ../results/fileio/sysbench_fileio_"$m"_"$t".txt ; 
done ; done
...
# seqrewr / block = 512
for m in seqrewr ; do for t in 1 2 4 6 8 12 16 24 32 48 64 98 128 190 256 384 512 ;     do echo "Test: $m threads: $t" ; 
...
# direct / rw ratio = 4
for m in rndrw ; do for t in 1 2 4 6 8 12 16 24 32 48 64 98 128 190 256 384 512;       do echo "Test: $m threads: $t" ; 
