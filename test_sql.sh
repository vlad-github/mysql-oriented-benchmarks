...
for t in 1 2 4 6 8 12 16 24 32 48 64 96 128 160 192 224 256 288 320 352 384 416 448 480 512 ; do echo -n "RO/treads=$t:\t" ; sysbench --test=/usr/share/sysbench/oltp_point_select.lua run --mysql-socket=$MYSQL_SOCK --mysql-user=$MYSQL_USER --mysql-password=$MYSQL_PASS --tables=64 --table-size=10000000 --threads=$t --max-time=100 > ./results/sql/oltp_point_select_"$t".txt ; sleep 10 ; done
for t in 1 2 4 6 8 12 16 24 32 48 64 96 128 160 192 224 256 288 320 352 384 416 448 480 512 ; do echo -n "RW/threads=$t:\t" ; sysbench --test=/usr/share/sysbench/oltp_read_write.lua run --mysql-socket=$MYSQL_SOCK --mysql-user=$MYSQL_USER --mysql-password=$MYSQL_PASS --tables=64 --table-size=10000000 --threads=$t --max-time=100 > ./results/sql/oltp_read_write_"$t".txt ; sleep 10 ; done
